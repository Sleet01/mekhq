...
public class IntelRating {
    ...
    public void loadFieldsFromXmlNode(Campaign campaign, Version version, Node node) throws ParseException {
        ...
        try {
            ...
            // personnel map.
            updateAssignedPersons(getPersonnelMap(campaign));
        } catch (Exception e) {
            LOGGER.error("", e);
        }
    }

    public static Map<UUID, Person> getPersonnelMap(Campaign campaign) {
        // Create new UUID:Person map from all personnel
        HashMap<UUID, Person> personnel = new HashMap<UUID, Person>();
        campaign.getPlayerForce().allPersonnel().forEach(p -> personnel.put(p.getId(), p));
        return personnel;
    }
}
